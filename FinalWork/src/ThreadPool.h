...
public:
    ...
    bool empty()
    {
        std::unique_lock<std::mutex> lock(m_mutex); // 加锁，防止队列被修改
    ...
    void enqueue(T &t)
    {
        std::unique_lock<std::mutex> lock(m_mutex); // 添加元素同样需要加锁
        m_queue.emplace(t);
    }
    // 队列取出元素
    bool dequeue(T &t)
    {
        ...
        if (m_queue.empty())
            return false;
        /*
            这里将队列中的元素进行右值引用，即将队列中的元素进行移动操作，避免了拷贝构造函数的调用
        */
...
private:
    ...
    public:
        ...
        void operator()()
        {
            ...
            /*
                在线程池处于工作时循环从任务队列中提取任务，并利用条件变量，在任务队列为空时阻塞当前线程。
                当任务队列中有任务时，提取并执行任务。
            */
